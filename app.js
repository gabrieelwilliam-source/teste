(()=>{
  ...
  function healthClass(adherence,excess,confirmed){const rate=excess/Math.max(confirmed,1);return adherence>=.9&&rate<=.05?'good':adherence>=.78&&rate<=.15?'warn':'bad'}
  function healthLabel(adherence,excess,confirmed){const c=healthClass(adherence,excess,confirmed);return c==='good'?'BOM':c==='warn'?'ATENÇÃO':'CRÍTICO'}
  const emptyState=(title='Sem dados neste filtro',text='Ajuste o período ou os filtros para visualizar informações.')=>`<div class="empty-state"><strong>${title}</strong><span>${text}</span></div>`;
  ...
  function renderOverview(){
    ...
    const totalStores=new Set([...data.state.map(r=>r.cliente),...data.sellers.flatMap(s=>s.lojas||[])]).size||20;
    $('#kpiOrders').textContent=fmt(orders);$('#kpiSuggested').textContent=fmt(suggested);$('#kpiExcess').textContent=fmt(excess);$('#kpiExcessPct').textContent=`${pct(excess/Math.max(orders,1))} do pedido realizado`;$('#kpiReturns').textContent=fmt(returnTotal);$('#kpiSales').textContent=fmt(sales);$('#kpiAdherence').textContent=pct(adherence);$('#kpiVisits').textContent=fmt(new Set(rows.map(r=>r.cliente)).size);$('#kpiVisitsSub').textContent=`de ${totalStores} lojas cadastradas`;
    const sellerAgg=aggBy(rows,'vendedor').sort((a,b)=>b.excess-a.excess),storeAgg=aggBy(rows,'cliente').sort((a,b)=>b.excess-a.excess),worst=sellerAgg[0],worstStore=storeAgg[0];
    const alert=$('#executiveAlert'),problem=excess>Math.max(30,orders*.05);
    if(!rows.length){alert.className='executive-alert';alert.innerHTML=`<div class="alert-icon">i</div><div><span class="section-label">RESUMO DO PERÍODO</span><h3>Ainda não há movimentações para estes filtros</h3><p>Escolha outro período, vendedor, loja ou produto para consultar os registros disponíveis.</p></div>`}
    else if(problem){alert.className='executive-alert';alert.innerHTML=`<div class="alert-icon">!</div><div><span class="section-label">O QUE PRECISA DE ATENÇÃO</span><h3>${fmt(excess)} unidades foram pedidas acima da recomendação</h3><p>Comece revisando ${worstStore?`<b>${esc(worstStore.name)}</b>`:'as lojas com maior excesso'}${worst?` e <b>${esc(worst.name)}</b>`:''}. Abaixo estão as prioridades em ordem de impacto.</p></div><div class="alert-score"><strong>${fmt(returnTotal)}</strong><span>trocas / devoluções</span></div>`}
    else{alert.className='executive-alert good';alert.innerHTML=`<div class="alert-icon">✓</div><div><span class="section-label">SITUAÇÃO GERAL</span><h3>Os pedidos estão próximos do recomendado</h3><p>A aderência está em <b>${pct(adherence)}</b> e o excesso está controlado para o período selecionado.</p></div><div class="alert-score"><strong>${fmt(returnTotal)}</strong><span>trocas / devoluções</span></div>`}
  ...
  function renderEfficiency(rows){
    const adherence=avg(rows,'aderencia'),orders=sum(rows,'pedidoConfirmado'),excess=sum(rows,'excessoPotencial'),ret=sum(rows,'trocas'),sales=sum(rows,'vendaEstimada'),excessRate=excess/Math.max(orders,1),returnRate=ret/Math.max(sales+ret,1);
    const totalStores=new Set([...data.state.map(r=>r.cliente),...data.sellers.flatMap(s=>s.lojas||[])]).size||20,visited=new Set(rows.map(r=>r.cliente)).size,coverage=visited/Math.max(totalStores,1);
    const metrics=[['Pedidos próximos do recomendado',adherence,adherence>=.9?'green':adherence>=.78?'orange':'red',pct(adherence)],['Pedido sem excesso',Math.max(0,1-excessRate),excessRate<=.05?'green':excessRate<=.15?'orange':'red',pct(Math.max(0,1-excessRate))],['Baixa taxa de trocas',Math.max(0,1-returnRate*5),returnRate<=.03?'green':returnRate<=.07?'orange':'red',pct(returnRate)+' de trocas'],['Lojas acompanhadas',coverage,coverage>=.8?'green':coverage>=.5?'orange':'red',`${visited}/${totalStores}`]];
  ...
  function renderTopRisks(rows){const groups=aggBy(rows,'cliente').filter(g=>g.excess>0).sort((a,b)=>b.excess-a.excess).slice(0,5);$('#topRisks').innerHTML=groups.length?groups.map((g,i)=>`<div class="risk-item" data-kind="store" data-name="${esc(g.name)}"><div class="risk-rank">${i+1}</div><div><b>${esc(g.name)}</b><small>${g.visits} visita${g.visits===1?'':'s'} · ${pct(g.adherence)} de aderência</small></div><div class="risk-value"><strong>+${fmt(g.excess)}</strong><span>unidades acima</span></div></div>`).join(''):emptyState('Nenhum excesso relevante','Os pedidos deste filtro estão próximos da recomendação.');$$('.risk-item').forEach(x=>x.onclick=()=>openEntity('store',x.dataset.name))}
  ...
  function renderLatest(rows){const vv=groupVisits(rows).slice(0,10);$('#latestVisits').innerHTML=vv.length?vv.map(v=>`<tr data-visit="${esc(v.id)}"><td>${dateTimeFmt(v.dataHora)}</td><td><b>${esc(v.vendedor)}</b></td><td>${esc(v.cliente)}</td><td>${fmt(v.suggested)}</td><td><b>${fmt(v.confirmed)}</b></td><td>${v.diff>0?'<b style="color:#b42318">+':''}${fmt(v.diff)}${v.diff>0?'</b>':''}</td><td><span class="status ${statusClass(v.diff,v.suggested)}">${statusClass(v.diff,v.suggested)==='ok'?'DENTRO DO ESPERADO':v.diff>0?'PEDIU ACIMA':'PEDIU ABAIXO'}</span></td></tr>`).join(''):`<tr><td colspan="7">${emptyState('Nenhuma movimentação encontrada','Não há visitas registradas dentro dos filtros selecionados.')}</td></tr>`;$$('#latestVisits tr[data-visit]').forEach(tr=>tr.onclick=()=>openVisit(vv.find(v=>v.id===tr.dataset.visit)))}
  function renderSellers(){const rows=filteredHistory(),ag=aggBy(rows,'vendedor').sort((a,b)=>b.excess-a.excess);$('#sellerCards').innerHTML=ag.length?ag.map(g=>{const hc=healthClass(g.adherence,g.excess,g.confirmed),hl=healthLabel(g.adherence,g.excess,g.confirmed);return `<article class="seller-card" data-name="${esc(g.name)}"><div class="seller-card-top"><div class="avatar">${esc(g.name.split(' ').pop())}</div><div><h3>${esc(g.name)}</h3><small>${(data.sellers.find(s=>s.vendedor===g.name)?.lojas||[]).join(' · ')}</small></div><span class="simple-status ${hc}">${hl}</span></div><div class="seller-score ${g.adherence>=.9?'good':g.adherence>=.78?'warn':''}">${pct(g.adherence)}</div><span class="section-label">PEDIDOS PRÓXIMOS DA RECOMENDAÇÃO</span><div class="mini"><span><b>${g.visits}</b> visitas</span><span><b>${fmt(g.confirmed)}</b> pedidos</span><span><b>${fmt(g.excess)}</b> excesso</span></div></article>`}).join(''):emptyState('Sem vendedores neste filtro','Altere o período ou remova algum filtro.');$$('.seller-card').forEach(c=>c.onclick=()=>openEntity('seller',c.dataset.name));$('#sellerTable').innerHTML=ag.length?ag.map(g=>`<tr data-name="${esc(g.name)}"><td><b>${esc(g.name)}</b></td><td>${(data.sellers.find(s=>s.vendedor===g.name)?.lojas||[]).join(', ')}</td><td>${g.visits}</td><td>${fmt(g.suggested)}</td><td>${fmt(g.confirmed)}</td><td>${fmt(g.excess)}</td><td>${pct(g.adherence)}</td><td>${fmt(g.returns)}</td></tr>`).join(''):`<tr><td colspan="8">Sem registros para os filtros atuais.</td></tr>`;$$('#sellerTable tr[data-name]').forEach(c=>c.onclick=()=>openEntity('seller',c.dataset.name))}
  function renderEntities(kind){const rows=filteredHistory(),key=kind==='store'?'cliente':'produto',target=kind==='store'?$('#storeGrid'):$('#productGrid');const ag=aggBy(rows,key).sort((a,b)=>b.excess-a.excess);target.innerHTML=ag.length?ag.map(g=>{const hc=healthClass(g.adherence,g.excess,g.confirmed),hl=healthLabel(g.adherence,g.excess,g.confirmed);return `<article class="entity-card" data-name="${esc(g.name)}" data-kind="${kind}"><div class="entity-topline"><div><span>${kind==='store'?'LOJA':'PRODUTO'}</span><h3>${esc(g.name)}</h3></div><span class="simple-status ${hc}">${hl}</span></div><div class="entity-kpis"><div><span>Pedido realizado</span><strong>${fmt(g.confirmed)}</strong></div><div><span>Excesso potencial</span><strong>${fmt(g.excess)}</strong></div><div><span>Aderência</span><strong>${pct(g.adherence)}</strong></div><div><span>Trocas</span><strong>${fmt(g.returns)}</strong></div></div></article>`}).join(''):emptyState('Sem dados para mostrar','Ajuste os filtros para visualizar os registros.');target.querySelectorAll('.entity-card').forEach(c=>c.onclick=()=>openEntity(kind,c.dataset.name))}
  function renderHistory(){const rows=filteredHistory().sort((a,b)=>dateMs(b.dataHora)-dateMs(a.dataHora)).slice(0,600);$('#historyTable').innerHTML=rows.length?rows.map(r=>`<tr><td>${dateFmt(r.dataHora)}</td><td>${esc(r.vendedor)}</td><td>${esc(r.cliente)}</td><td>${esc(r.produto)}</td><td>${fmt(r.estoqueContado)}</td><td>${fmt(r.vendaEstimada)}</td><td>${fmt(r.sugestao)}</td><td><b>${fmt(r.pedidoConfirmado)}</b></td><td>${r.diferencaPedido>0?'+':''}${fmt(r.diferencaPedido)}</td><td>${pct(r.aderencia)}</td><td>${fmt(r.trocas)}</td></tr>`).join(''):`<tr><td colspan="11">Nenhuma movimentação encontrada para os filtros atuais.</td></tr>`}
  ...
  function renderAlerts(){const aa=alerts();els.alertBadge.textContent=aa.length;$('#alertsList').innerHTML=aa.length?aa.slice(0,100).map((a,i)=>`<article class="alert-card ${a.level}" data-i="${i}"><div><span class="section-label">${a.level==='high'?'PRIORIDADE ALTA':'REVISAR'}</span><h3>${esc(a.title)}</h3><p>${esc(a.text)} · ${dateFmt(a.row.dataHora)}</p></div><aside><strong>${esc(a.meta)}</strong><span>clique para detalhes</span></aside></article>`).join(''):emptyState('Tudo sob controle','Não existem alertas relevantes dentro dos filtros selecionados.');$$('.alert-card').forEach(x=>x.onclick=()=>openRow(aa[Number(x.dataset.i)].row))}
  ...
  const analysisDetails=$('#analysisDetails');if(analysisDetails)analysisDetails.addEventListener('toggle',()=>{if(analysisDetails.open)setTimeout(()=>{renderSellerChart(aggBy(filteredHistory(),'vendedor').sort((a,b)=>b.excess-a.excess));renderTrend(filteredHistory())},30)});
